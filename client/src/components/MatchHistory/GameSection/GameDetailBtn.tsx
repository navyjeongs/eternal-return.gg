...
import { AddDetail, OpenDetail } from "../../../pages/MatchHistory/MatchHistory";

interface Props {
  isOpenDetail: AddDetail;
  setIsOpenDetail: React.Dispatch<SetStateAction<OpenDetail>>;
  gameId: number;
}

interface HandleClickDetailBtn {
  (e: React.MouseEvent<HTMLButtonElement>, gameId: number): void;
}
...
const GameDetailBtn = ({ isOpenDetail, setIsOpenDetail, gameId }: Props) => {
  const btnRef = useRef<HTMLButtonElement | null>(null);

  const [isClickBtn, setIsClickBtn] = useState<boolean>(false);

  // 게임 기록 자세히 불러오기
  const loadDetailGameInfo: HandleClickDetailBtn = async (e, gameId) => {
    ...
    if (!isOpenDetail.isLoad) {
      ...
      setIsOpenDetail(
        produce((draft: Draft<OpenDetail>) => {
          draft[gameId] = {
            isOpen: true,
            isLoad: true,
  ...
  return (
    <Container>
      <DetailBtn ref={btnRef} onClick={(e) => loadDetailGameInfo(e, gameId)}>
